...
const gcStorage = new Storage({
  ...
  }, // Ganti dengan lokasi kunci akses JSON Anda
...
const createmovies = async (moviesData) => {
  try {
    ...
    const moviesFileName = `${Date.now()}_${movies.originalname}`;
...
const updatemovies = async (moviesId, moviesData) => {
  try {
    ...
    if (image) {
      if (movies.image) {
        const imageBucket = gcStorage.bucket("moviesdata"); // Ganti dengan nama bucket penyimpanan gambar
        const imageFileName = movies.image.split("/").pop();
        const imageFile = imageBucket.file(imageFileName);
    ...
    if (newmovies) {
      if (Movies.movies) {
        const moviesBucket = gcStorage.bucket("moviesdata"); // Ganti dengan nama bucket penyimpanan movies
        const moviesFileName = Movies.movies.split("/").pop();
        const moviesFile = moviesBucket.file(moviesFileName);
    ...
    let moviesUpdated = Movies.movies;
    if (newmovies) {
      const moviesBucket = gcStorage.bucket("moviesdata"); // Ganti dengan nama bucket penyimpanan movies
      const moviesFileName = `${Date.now()}_${movies.originalname}`;
      const moviesFile = moviesBucket.file(moviesFileName);
    ...
    Movies.nama = nama || Movies.nama;
    Movies.deskripsi = deskripsi || Movies.deskripsi;
    Movies.tanggal = tanggal || Movies.tanggal;
    Movies.genre = genre || Movies.genre;
    Movies.image = imageUpdated;
    Movies.movies = moviesUpdated;
